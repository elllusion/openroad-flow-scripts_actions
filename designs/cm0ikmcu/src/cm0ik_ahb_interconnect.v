...
// 1 master port and 7 slave ports:
// Master   -   AHBLite from Cortex-M0 / Cortex-M0 Integration Timing Wrapper
// Slave 0  -   ROM
// Slave 1  -   RAM
// Slave 2  -   GPIO 0
// Slave 3  -   GPIO 1
// Slave 4  -   GPIO 2
// Slave 5  -   System ROM Table
// Slave 6  -   AHB Default Slave
//-----------------------------------------------------------------------------

module cm0ik_ahb_interconnect
  (// Inputs
   input wire           HCLK,
   input wire           HRESETn,      
   ...
   input wire           HRESPS5,
   input wire           HREADYOUTS6,
   input wire [31:0]    HRDATAS6,
   input wire           HRESPS6,
   // Output wires
   output wire          HREADY,
   output wire          HRESP,
  ...
  wire  hsel_s0 = (HADDR[31:20] == 12'h000);

  // RAM Select: 0x2000_0000 to 0x200F_FFFF 
  wire  hsel_s1 = (HADDR[31:20] == 12'h200);

  // GPIO 0 : 0x4000_0000 to 0x4000_07FF  
  wire  hsel_s2 = (HADDR[31:11] == 21'b0100_0000_0000_0000_0000_0); 
  
  // GPIO 1 : 0x4000_0800 to 0x4000_0FFF  
  wire  hsel_s3 = (HADDR[31:11] == 21'b0100_0000_0000_0000_0000_1); 

  // GPIO 2 : 0x4000_1000 to 0x4000_17FF  
  wire  hsel_s4 = (HADDR[31:11] == 21'b0100_0000_0000_0000_0001_0); 

  // System ROM Table: 0xF000_0000 to 0xF000_1FFF
  wire  hsel_s5 = (HADDR[31:13] == 19'b1111_0000_0000_0000_000); 

  // Everything else goes to AHB Default Slave
  wire  hsel_s6 = ~(hsel_s0 | hsel_s1 | hsel_s2 | hsel_s3 | hsel_s4 | hsel_s5);
  ...
  always @ (posedge HCLK or negedge HRESETn)
    ...
    wire        hready_m = (hsel_s0_r & HREADYOUTS0) |
                           ...
                           (hsel_s6_r & HREADYOUTS6) |
                           // Reply READY if no slave selected (reset)
                           (~(hsel_s0_r | hsel_s1_r | hsel_s2_r | hsel_s3_r | hsel_s4_r | hsel_s5_r | hsel_s6_r));
